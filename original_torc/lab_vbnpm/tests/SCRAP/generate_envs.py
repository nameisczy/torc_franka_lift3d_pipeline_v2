"""
automatically generate environments
"""
import mujoco
import numpy as np
import sys
...
def sample_obj_poses(filename):
    ...
    obj_poses = []
    # min: pos - half_size
    workspace_x_min = 1.1 + 0.0 - 0.25 + 0.04
    workspace_y_min = 0.0 + 0.0 - 0.58 + 0.19 + 0.08 - 0.04
    workspace_x_max = 1.1 + 0.0 + 0.25 - 0.18
    workspace_y_max = 0.0 + 0.0 + 0.58 - 0.19 - 0.08 - 0.04
    workspace_z_min = 1.1
    ...
    while i < n_objs:
        ...
        obj_angle = np.random.uniform(-np.pi, np.pi)
        if mj_model.body(i).name == '008_pudding_box':
            obj_orientation = tf.rotation_matrix(np.pi/2, [0, 1, 0])
            obj_orientation = np.dot(tf.rotation_matrix(obj_angle, [0, 0, 1]), obj_orientation)
        else:
            obj_orientation = tf.rotation_matrix(obj_angle, [0, 0, 1])
        obj_quat = tf.quaternion_from_matrix(obj_orientation)  # w,x,y,z
        ...
        in_collision = False
        for geom_id2 in geom_ids:
            fromto = np.zeros((6))
            dist_max = 0.002
            distance = mujoco.mj_geomDistance(mj_model, mj_data, geom_id, geom_id2, dist_max, fromto)
            if distance < dist_max:
                print("collision between geom %s and %s" % (mj_model.geom(geom_id).name, mj_model.geom(geom_id2).name))
