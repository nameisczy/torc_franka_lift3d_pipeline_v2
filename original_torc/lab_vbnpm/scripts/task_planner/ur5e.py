...
class Ur5e:
    ...
    def __init__(self, is_sim, ground_truth=False):
        ...
        self.suction_link = "ur5e_ee"
        self.gripper_group = "arm"
        # self.camera_link = "camera_arm_link"
        # self.camera_axis = (0, 1, 0)
        # self.camera_axis = (0, 0, 1)

        self.ignore_collision_ee_links = ["ur5e_ee", "flang", "tool0"]

        rp = RosPack()
        self.urdf = rp.get_path("lab_vbnpm")
        self.urdf += "/robots/ur5e/curobo/ur5e.urdf"
        self.curobo_root = rp.get_path("lab_vbnpm")
        self.curobo_root += "/robots/ur5e/curobo/"
        content_path = ContentPath(
            robot_config_absolute_path=self.curobo_root + "ur5e.yml",
            robot_urdf_absolute_path=self.curobo_root + "ur5e.urdf",
            robot_usd_absolute_path=self.curobo_root + "ur5e.usd",
