...
class DenseExecutionVideoRecorder:
    def __init__(self):
        self.enabled = os.environ.get("TORC_RENDER_EXECUTION_VIDEO") == "1"
        ...
        self.stride = max(1, int(os.environ.get("TORC_RENDER_STRIDE", "5")))
        ...
        if cameras:
            self.camera_names = [c.strip() for c in cameras.split(",") if c.strip()]
        else:
            self.camera_names = [os.environ.get("TORC_RENDER_CAMERA", "back_view")]

        self.video_dir = None

    def _log(self, *parts):
        print("[DENSE_EXEC_VIDEO]", *parts, flush=True)

    def _safe_camera_name(self, name):
        return "".join(ch if ch.isalnum() or ch in ("-", "_") else "_" for ch in name)

    def setup(self, model, experiment_dir):
        if not self.enabled:
            self._log("disabled")
            return
        if experiment_dir is None:
            self._log("enabled but experiment_dir is None; disabling recorder")
            self.enabled = False
            return
        ...
        for camera_name in self.camera_names:
            ...
            if camera_id < 0:
                self._log(f"camera not found: {camera_name}; skipping")
                self.failed_cameras.add(camera_name)
                continue

            safe_name = self._safe_camera_name(camera_name)
            video_path = os.path.join(
                self.video_dir, f"full_robot_continuous_{safe_name}.mp4"
            )
            ...
            if not writer.isOpened():
                self._log(f"failed to open writer for {camera_name}: {video_path}")
                self.failed_cameras.add(camera_name)
                continue
        ...
        self._log(
            "enabled",
            f"cameras={list(self.writers.keys())}",
    ...
    def capture(self, data):
        ...
        for camera_name in list(self.writers.keys()):
            if camera_name in self.failed_cameras:
                continue
            try:
                renderer = self.renderers[camera_name]
                writer, _ = self.writers[camera_name]
                renderer.update_scene(data, camera=self.camera_ids[camera_name])
                rgb = renderer.render()
    ...
    def close(self):
        if not self.initialized:
            return
        for camera_name, (writer, video_path) in list(self.writers.items()):
            try:
                writer.release()
            except Exception as exc:
                self._log(f"writer release failed for {camera_name}: {repr(exc)}")
            self._log(
                "closed",
                f"camera={camera_name}",
                f"frames={self.frame_counts.get(camera_name, 0)}",
                f"path={video_path}",
            )
        for camera_name, renderer in list(self.renderers.items()):
            try:
                renderer.close()
            except Exception as exc:
                self._log(f"renderer close failed for {camera_name}: {repr(exc)}")
        self.initialized = False
        self.renderers = {}
        self.writers = {}
        self.camera_ids = {}
...
class ExecutionNode:
    ...
    def __init__(self, address: str = "tcp://*:5858"):
        ...
        self.dense_video_recorder = None
    ...
    def reset(
        ...
        with self.mj_lock:
            ...
            if self.dense_video_recorder is not None:
                self.dense_video_recorder.close()
                self.dense_video_recorder = None
            ...
            self.dense_video_recorder = DenseExecutionVideoRecorder()
            self.dense_video_recorder.setup(self.model, self.experiment_dir)
    ...
    def step(self, sync_viewer: bool = True):
        ...
        with self.mj_lock:
            mujoco.mj_step(self.model, self.data)
            if self.dense_video_recorder is not None:
                self.dense_video_recorder.capture(self.data)
    ...
    def poll_socket(self):
        ...
        if cmd == "reset":
            reset_t0 = time.time()
            print(
                "[MOTOMAN_RESET_STAGE] reset begin",
                f"xml={req.get('args', {}).get('xml_file')}",
                f"experiment_dir={req.get('args', {}).get('experiment_dir')}",
                f"mj_pickle={req.get('args', {}).get('mj_pickle')}",
                flush=True,
            )
            self.reset(**req["args"])
            print(
                "[MOTOMAN_RESET_STAGE] reset returned",
                f"elapsed={time.time() - reset_t0:.3f}s",
                flush=True,
            )
            warmup_t0 = time.time()
            for i in range(1000):
                if i % 100 == 0:
                    print(
                        "[MOTOMAN_RESET_STAGE] warmup step",
                        f"i={i}",
                        f"elapsed={time.time() - warmup_t0:.3f}s",
                        flush=True,
                    )
                self.step(False)
            print(
                "[MOTOMAN_RESET_STAGE] warmup done",
                f"elapsed={time.time() - warmup_t0:.3f}s",
                flush=True,
            )
            print("[MOTOMAN_RESET_STAGE] sending success reply", flush=True)
            self.socket_reply("success")
            print(
                "[MOTOMAN_RESET_STAGE] success reply sent",
                f"total_elapsed={time.time() - reset_t0:.3f}s",
                flush=True,
            )
        else:
...
if __name__ == "__main__":
    ...
    atexit.register(
        lambda: execution_node.dense_video_recorder.close()
        if execution_node.dense_video_recorder is not None
        else None
    )
    if args.scene:
