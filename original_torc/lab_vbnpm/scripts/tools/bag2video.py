...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert ROS1 bag image topic to MP4 video"
    )
    parser.add_argument(
        "bag_files",
        nargs="+",
    ...
    parser.add_argument(
        "output_directory", help="Directory to save the output MP4 videos"
    )
    ...
    for bag_file in args.bag_files:
        ...
        for path in bag_paths:
            output_video = (
                f"{args.output_directory}/{path.split('/')[-1].replace('.bag', '.mp4')}"
            )
