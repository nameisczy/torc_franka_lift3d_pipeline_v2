...
class RobotAdapter:
    ...
    def _frame_from(self, approach_direction, grasp_axis):
        ...
        y_seed = np.asarray(grasp_axis, dtype=float).reshape(3)
        y_seed = y_seed / np.linalg.norm(y_seed)
    ...
    def adapt(self, grasp: CanonicalGrasp) -> RobotGraspCommand:
        ...
        opening = min(max(requested, 0.0), self.validated_open_width)
